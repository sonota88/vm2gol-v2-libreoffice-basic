...
function is_label_line(line_)
    is_label_line = (left(line_, 5) = "label")
end function
...
function _make_label_addr_map(lines)
    ...
    for ri = 0 to (lines.len - 1)
      line_ = List.get_(lines, ri)

      if is_label_line(line_) then
        v1 = split(line_, " ")(1)
...
sub _assemble(lines, label_addr_map)
    ...
    for ri = 0 to (lines.len - 1)
        ...
        if not is_label_line(line_) then
          line_ = substring(line_, 2)
        end if

        xs = List.from_array(split(line_, " "))
        v0 = List.get_(xs, 0)
